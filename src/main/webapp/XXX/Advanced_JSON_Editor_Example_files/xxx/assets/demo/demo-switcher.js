$(function() {
	...
	//Show Switcher
		...
		//collapse buttons		
		//Switch: collapse buttons		
		$('input[name="demo-collapse-buttons"]').on('switchChange.bootstrapSwitch', function(event, state) {
			// JSONEditor.defaults.options.show_errors = this.value;
			//alert (JSONEditor.defaults.options["disable_collapse"]);
			//JSONEditor.options.disable_collapse = !JSONEditor.options.disable_collapse; 
			JSONEditor.defaults.options["disable_collapse"] = !JSONEditor.defaults.options["disable_collapse"] ;
			// init / reload
			initJSONEditor();
		});
